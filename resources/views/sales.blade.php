...
<x-adminlte-modal id="view" size="lg" title="Información de Venta">
    ...
    <div class="row">
        <x-adminlte-button label="Cerrar" theme="secondary" data-dismiss="modal"/>
    </div>
</x-adminlte-modal>
...
<script>
    ...
    $(document).on("click", "#showInfo", async function() {
        ...
        $("#view #fecIngreso").text(dataSale[0].FEC_INGRESO.split('T')[0] + " " + dataSale[0].FEC_INGRESO.split('T')[1].split('.')[0]);
        $("#view #fecActualizacion").text(dataSale[0].FEC_ACTUALIZACION.split('T')[0] + " " + dataSale[0].FEC_ACTUALIZACION.split('T')[1].split('.')[0]);
